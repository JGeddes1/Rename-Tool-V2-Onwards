        ...
        if filenames_new != filenames:
            ...
            for i in range(0, filelength):
                ...
                if filenames_new[i] != filenames[i]:
                    ...
                    my_listbox2.itemconfig(numberFilesAlreadyGone2, {'fg': 'green'})

                    my_listbox.itemconfig(numberFilesAlreadyGone2, {'fg': 'red'})
        if filenames == filenames_new :
            exec('Label%d=Label(root,text="No Changes needed")\nLabel%d.grid(row=3, column=0)' %
                      (1000000, 1000000))
            for i in range(0, filelength):
                numberFilesAlreadyGone2 = numberFilesAlreadyGone2 + 1
                print("filenames new:", i, ":", filenames_new[i])
...
myButton = Button(root, text="Select Directory To Check",
                  command=myClick)
...
c2 = Checkbutton(root, text="subdir", variable=var2).grid(
    column=2, row=1, padx=5, pady=5, sticky='nsew')
